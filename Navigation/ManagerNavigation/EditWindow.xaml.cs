...
namespace Coursework
{
    public partial class EditWindow : Window, INotifyPropertyChanged
    {
        ...
        {
            _windowId = Convert.ToInt32(txtWindowId.Text);
            _newLength = Convert.ToDouble(txtLength.Text);
            _newWidth = Convert.ToDouble(txtWidth.Text);
            ...
            try
            {
                var result = (
                    ...
                    where mc.CalculationId == _intCalculationId && (fm.FeaturesId == 1 || fm.FeaturesId == 2)
                    select new
                    {
        ...
        private double CalculateTotal()
        {
            ...
            var windowToUpdate = _context.Windows.FirstOrDefault(w => w.WindowId == _windowId);
            if (windowToUpdate != null)
            {
                windowToUpdate.Length = (decimal)_newLength;
                windowToUpdate.Width = (decimal)_newWidth;
                windowToUpdate.Count = (byte)_newCount;
                _context.SaveChanges();
            }
            ...
            return (double)result.RequiredSidingArea;
        }
